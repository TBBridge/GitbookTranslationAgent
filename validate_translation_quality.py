...
This script validates translation quality by:
1. Generating sample translations using the GitBook Translator
2. Verifying format preservation
3. Verifying glossary application
4. Verifying link and image preservation
...
class TranslationQualityValidator:
    ...
    def generate_sample_translations(self, glossary_path: str) -> Dict[str, str]:
        """Generate sample translations for validation."""
        print("\n=== Generating Sample Translations ===")
        
        # Sample content with various GitBook features
        sample_content = """---
title: テストドキュメント
...
"""
        ...
        # Mock translation results for different languages
        translations = {}
        
        # English translation (simulated with proper format preservation)
        translations['en'] = """---
title: Test Document
...
"""
        ...
        # Chinese Simplified translation (simulated)
        translations['zh-CN'] = """---
title: 测试文档
...
"""
        ...
        # Save sample translations to files
        for lang, content in translations.items():
            output_path = Path(self.temp_dir) / f"sample.{lang}.md"
            with open(output_path, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"Generated sample translation: {output_path}")
        
        # Also save original for comparison
        original_path = Path(self.temp_dir) / "sample.ja.md"
        with open(original_path, 'w', encoding='utf-8') as f:
            f.write(sample_content)
        print(f"Saved original content: {original_path}")
    ...
    def validate_format_preservation(self, original: str, translated: str, language: str) -> Dict[str, Any]:
        ...
        # Check inline code preservation (more precise pattern)
        ...
        gitbook_tags = [r'{% hint.*?%}', r'{% endhint %}', r'{% tabs %}', r'{% endtabs %}', 
                       r'{% tab.*?%}', r'{% endtab %}', r'{% include.*?%}', r'{{.*?}}']
        
        for tag_pattern in gitbook_tags:
            ...
            if original_tags != translated_tags:
                results['issues'].append(f"GitBook tag mismatch for pattern {tag_pattern}")
                results['passed'] = False
        ...
        original_tables = re.findall(r'\|.*?\|', original)
        translated_tables = re.findall(r'\|.*?\|', translated)
        
        if len(original_tables) != len(translated_tables):
            results['issues'].append(f"Table row count mismatch: {len(original_tables)} vs {len(translated_tables)}")
            results['passed'] = False
        else:
            # Check column count consistency
            for i, (orig_row, trans_row) in enumerate(zip(original_tables, translated_tables)):
                orig_cols = orig_row.count('|')
                trans_cols = trans_row.count('|')
                if orig_cols != trans_cols:
                    results['issues'].append(f"Table row {i+1} column count mismatch: {orig_cols} vs {trans_cols}")
                    results['passed'] = False
            if results['passed']:
                print(f"✓ Table structure preserved ({len(original_tables)} rows)")
            ...
            if expected_translation:
                ...
                if expected_translation in translated:
                    ...
                    if ja_term in translated:
                        ...
                        # Term might have been translated differently or not present in source
                        print(f"? Glossary term '{ja_term}' -> '{expected_translation}' not found (may not be in source)")
        
        # Check for consistency - same Japanese term should always translate to same target term
        term_usage = {}
        for term in glossary['terms']:
            ja_term = term['ja']
            expected_translation = term.get(language, '')
            if expected_translation and expected_translation in translated:
                if ja_term in term_usage:
                    if term_usage[ja_term] != expected_translation:
                        results['issues'].append(f"Inconsistent translation for '{ja_term}': {term_usage[ja_term]} vs {expected_translation}")
                        results['passed'] = False
                else:
                    term_usage[ja_term] = expected_translation
        
        if results['passed']:
            print("✓ Glossary terms consistently applied")
    ...
    def validate_link_preservation(self, original: str, translated: str, language: str) -> Dict[str, Any]:
        ...
        # Extract links from original and translated
        link_pattern = r'\[([^\]]*)\]\(([^)]+)\)'
    ...
    def validate_overall_quality(self, original: str, translated: str, language: str) -> Dict[str, Any]:
        ...
        # Remove protected regions first
        ...
        if japanese_chars:
            results['issues'].append(f"Untranslated Japanese text found: {japanese_chars[:5]}")  # Show first 5
            results['passed'] = False
        else:
    ...
    def run_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            translations = self.generate_sample_translations(glossary_path)
        ...
        for category, results in self.validation_results.items():
            ...
            if total_issues > 0:
                ...
                for result in results:
                    if result['issues']:
                        print(f"    {result['language']}: {', '.join(result['issues'])}")
            else:
